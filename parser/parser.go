...
func (p *Parser) parseLetStatement() *ast.LetStatement { //Let语句的抽象语法树
	stmt := &ast.LetStatement{Token: p.curToken} //p.curToken应该是Let
...
func (p *Parser) parseExpression(precedence int) ast.Expression { //非常重要的函数，创建表达式的抽象语法树，如果是表达式的开始，一般使用LOWEST优先级
	...
	for !p.peekTokenIs(token.SEMICOLON) && precedence < p.peekPrecedence() { //如果下一个Token不是';'，而且优先级大于参数
		infix := p.infixParseFns[p.peekToken.Type] //寻找后缀解析函数
		if infix == nil {
			return leftExp
		}
